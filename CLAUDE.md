...
- **Secrets** : copier `.env.example` → `.env` (ignoré par git) et renseigner
  `ANTHROPIC_API_KEY` (requis pour `SlidingWindowCVRanker`) et `MISTRAL_API_KEY`
  (requis pour `SyntheticCVGenerator`). Chargé via `python-dotenv`.
...
- **`models/`** (wrappers de modèles) :
  ...
  - `synthetic_cv_generator.py` — `SyntheticCVGenerator` : génère des CVs
    ...
    `rank` de vérité-terrain). Entrées : `import_model()`, `generate_cv()`,
    `generate_cvs()` → écrit les PDF + un `manifest.json` sous
    `GENERATED_DATA_DIR/synthetic_cvs_<timestamp>/`.
...
- `generate_synthetic_cvs.py` — génère `--count N` CVs synthétiques PDF face à une
  ...
  (nécessite `MISTRAL_API_KEY`). Sorties sous `data/generated_data/synthetic_cvs_<timestamp>/`.
